...
class Encoder(nn.Module):
    ...
    def forward(self, img, imu):
        ...
        # print("img shape: ", img.shape) # (16, 11, 3, 256, 512)
        
        v = torch.cat((img[:, :-1], img[:, 1:]), dim=2) # (16, 10, 6, 256, 512)
        batch_size = v.size(0)
        seq_len = v.size(1)

        # image CNN
        v = v.view(batch_size * seq_len, v.size(2), v.size(3), v.size(4))
        # print("v shape: ",v.shape) # (160, 6, 256, 512)
        v = self.encode_image(v)
        # print("v shape: ",v.shape) # (160, 1024, 4, 8)
        v = v.view(batch_size, seq_len, -1)  # (batch, seq_len, fv)
        # print("v shape: ",v.shape) # (16, 10, 32768)
        v = self.visual_head(v)  # (batch, seq_len, 256)
        # print("v shape: ",v.shape) # (16, 10, 512)

        # IMU CNN
        imu = torch.cat([imu[:, i * 10:i * 10 + 11, :].unsqueeze(1) for i in range(seq_len)], dim=1)
        # print("imu  shape", imu.shape)  # (16, 10, 11, 6)
...
class Pose_RNN(nn.Module):
    ...
    def forward(self, fv, fv_alter, fi, dec, prev=None):
        ...
        Rt = self.layer_R(out).squeeze(1)      # (B, 9)
        
        R = torch.zeros(Rt.size(0), 6, 6).to(Rt.device)
        R[:, 0, 0] = Rt[:, 0] ** 2                          # sigma roll (X) 
        R[:, 1, 1] = Rt[:, 1] ** 2                          # sigma yaw (Y)
        R[:, 2, 2] = Rt[:, 2] ** 2                          # sigma pitch (Z)
        
        R[:, 3, 3] = Rt[:, 3] ** 2                          # sigma x
        R[:, 4, 4] = Rt[:, 4] ** 2                          # sigma y
        R[:, 5, 5] = Rt[:, 5] ** 2                          # sigma z
        
        R[:, 3, 5] = Rt[:, 6] * Rt[:, 3] * Rt[:, 5]         # raw1 * sigma x * sigma z
        R[:, 5, 3] = Rt[:, 6] * Rt[:, 3] * Rt[:, 5]         # raw1 * sigma x * sigma z
  
        R[:, 3, 1] = Rt[:, 7] * Rt[:, 3] * Rt[:, 1]         # raw2 * sigma x * sigma Y
        R[:, 1, 3] = Rt[:, 7] * Rt[:, 3] * Rt[:, 1]         # raw2 * sigma x * sigma Y        
        
        R[:, 1, 5] = Rt[:, 8] * Rt[:, 5] * Rt[:, 1]         # raw3 * sigma z * sigma Y
        R[:, 5, 1] = Rt[:, 8] * Rt[:, 5] * Rt[:, 1]         # raw3 * sigma z * sigma Y          
...
class DeepVIO(nn.Module):
    def __init__(self, opt):
        ...
        self.Feature_net = Encoder(opt)             # feature encoder (Visual + IMU)   
        self.Pose_net = Pose_RNN(opt)               # Pose Regressor
        self.Policy_net = PolicyNet(opt)            # decision making 
        self.opt = opt                              # 
    ...
    def forward(self, img, imu, is_first=True, hc=None, temp=5, selection='gumbel-softmax', p=0.5):
        # print("img shape: ", img.shape) # (16, 11, 3, 256, 512)
        # print("imu shape: ", imu.shape) # (16, 101, 6)
        ...
        for i in range(seq_len):
            if i == 0 and is_first:
                ...
                pose, R, hc = self.Pose_net(fv[:, i:i+1, :], None, fi[:, i:i+1, :], None, hc)
            else:
                if selection == 'gumbel-softmax':
                    # Otherwise, sample the decision from the policy network
                    p_in = torch.cat((fi[:, i, :], hidden), -1)
                    logit, decision = self.Policy_net(p_in.detach(), temp)          # decision (16, 2)
                    decision = decision.unsqueeze(1)                                # decision (16, 1, 2)
                    logit = logit.unsqueeze(1)
                    pose, R, hc = self.Pose_net(fv[:, i:i+1, :], fv_alter[:, i:i+1, :], fi[:, i:i+1, :], decision, hc)
        ...
        R_new = torch.cat(Rts, dim=1)
        decisions = torch.cat(decisions, dim=1)
        logits = torch.cat(logits, dim=1)
        probs = torch.nn.functional.softmax(logits, dim=-1)

        return poses, R_new, decisions, probs, hc, fi
